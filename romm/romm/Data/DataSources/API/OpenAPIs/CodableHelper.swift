...
// MARK: - Flexible Date Decoding Extension
extension KeyedDecodingContainer {
    /// Flexibly decode Date from either ISO string or timestamp
    func decodeFlexibleDate(forKey key: K) throws -> Date {
        if let dateString = try? self.decode(String.self, forKey: key) {
            return DateFormatter.iso8601Full.date(from: dateString) ?? Date()
        } else if let timestamp = try? self.decode(Double.self, forKey: key) {
            return Date(timeIntervalSince1970: timestamp)
        } else if let timestamp = try? self.decode(Int.self, forKey: key) {
